...
export default async function AIInsightsPage() {
  ...
  // Fără dateCondition — period = "all" pentru context complet
  const [categoryRows, monthRows, incomeRows, incomeMonthRows] = await Promise.all([
    db
      ...
      .where(and(eq(schema.transactions.userId, user.id), lt(schema.transactions.amount, 0)))
    ...
    db
      ...
      .from(schema.transactions)
      .where(and(eq(schema.transactions.userId, user.id), gt(schema.transactions.amount, 0))),
  ...
  return <AIInsightsClient initialData={initialData} />;
}
